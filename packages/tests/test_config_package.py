...
def test_config_package_initializes():
    ...
    assert ConfigPackage(section="server", option="host", value="127.0.0.1") is not None
    assert ConfigPackage(section="server", option="port", value=8000) is not None
    assert ConfigPackage(section="server", option="enabled", value=True) is not None
    assert ConfigPackage(section="server", option="interval", value=25.8) is not None
...
def test_config_package_converts_to_bytes_str():
    ...
    assert package.to_bytes() == bytes([
        0x00, 0x00, 0x00, 0x28,  # Package Size
        0x04,  # Identifier
        0x69, 0x0B, 0x08, 0x15,  # Timestamp (1762330645)
        0x00, 0x00, 0x00, 0x06,  # Section length
        0x00, 0x00, 0x00, 0x04,  # Option length
        0x00, 0x00, 0x00, 0x09,  # Value length
        0x73, 0x65, 0x72, 0x76, 0x65, 0x72,  # Section (server)
        0x68, 0x6f, 0x73, 0x74,  # Option (host)
        0x31, 0x32, 0x37, 0x2e, 0x30, 0x2e, 0x30, 0x2e, 0x31  # Value (127.0.0.1)
    ])


def test_config_package_converts_to_bytes_int():
    """Tests if the progress package converts to bytes with an int value."""
    package = ConfigPackage(1762330645, "server", "port", 8000)
    print(package.to_bytes().hex())
    assert package.to_bytes() == bytes([
        0x00, 0x00, 0x00, 0x23,  # Package Size
        0x04,  # Identifier
        0x69, 0x0B, 0x08, 0x15,  # Timestamp (1762330645)
        0x00, 0x00, 0x00, 0x06,  # Section length
        0x00, 0x00, 0x00, 0x04,  # Option length
        ...
        0x73, 0x65, 0x72, 0x76, 0x65, 0x72,  # Section (server)
        0x70, 0x6F, 0x72, 0x74,  # Option (port)
...
def test_config_package_converts_to_bytes_bool():
    ...
    assert package.to_bytes() == bytes([
        ...
        0x69, 0x0B, 0x08, 0x15,  # Timestamp (1762330645)
        0x00, 0x00, 0x00, 0x06,  # Section length
        0x00, 0x00, 0x00, 0x07,  # Option length
        0x00, 0x00, 0x00, 0x01,  # Value length
        0x73, 0x65, 0x72, 0x76, 0x65, 0x72,  # Section (server)
        0x65, 0x6E, 0x61, 0x62, 0x6C, 0x65, 0x64,  # Option (enabled)
        0x01  # Value (True)
    ])
...
def test_config_package_converts_to_bytes_float():
    ...
    assert package.to_bytes() == bytes([
        0x00, 0x00, 0x00, 0x27,  # Package Size
        0x04,  # Identifier
        0x69, 0x0B, 0x08, 0x15,  # Timestamp (1762330645)
        0x00, 0x00, 0x00, 0x06,  # Section length
        0x00, 0x00, 0x00, 0x08,  # Option length
        0x00, 0x00, 0x00, 0x04,  # Value length
        0x73, 0x65, 0x72, 0x76, 0x65, 0x72,  # Section (server)
        0x69, 0x6e, 0x74, 0x65, 0x72, 0x76, 0x61, 0x6c,  # Option (interval)
...
def test_bytes_converts_to_config_package_str():
    ...
    assert package.section == "server"
    assert package.option == "host"
